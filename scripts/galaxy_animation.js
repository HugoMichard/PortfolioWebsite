...
function interpolate4(a, b, p, r = { x: 0, y: 0, z: 0, w: 0 }) {
...
function render(now) {
    const galaxyCanvas = document.getElementsByClassName("galaxy-background");
    resize(galaxyCanvas)
    for(const galaxy of galaxyCanvas) {
        const context = galaxy.getContext("2d");
        const centerX = galaxy.width * 0.5,
        centerY = galaxy.height * 0.5;
        ...
        context.clearRect(0, 0, galaxy.width, galaxy.height);

        context.save();
        context.fillStyle = grd;
        context.fillRect(0, 0, galaxy.width, galaxy.height);
        context.restore();
        ...
        for (let arm = 0; arm < ARMS; arm++) {
            ...
            { x: 0.0, y: 0.25, z: 0.75, w: 0.0 },
            { x: 0.1, y: 0.5, z: 1.0, w: 0.5 },
            { x: 1.0, y: 0.75, z: 0.1, w: 0.8 },
